...
### 3.2 パーティ募集機能 (Role / Position Recruit — FF14 "Party Finder" スタイル)

| 項目 | 型 | 必須 | 説明 |
|------|----|------|------|
| ポジションスロット | `jsonb` | ✓ | 例: `{ "P":1, "C":1, "1B":0, ... }` 0=充足, 1=欠員 |
| 募集コメント | `text` |   | ポジションごとの希望 (例: "Pは経験3年以上") |
| 練度 | `integer` |   | チーム全体の平均経験年数目安 |
| スタイル | `enum` |   | `casual` / `competitive` |
| 試合リンク | `foreign_key -> Match` | ✓ | どの試合に紐づくか |

**UI イメージ**
* 募集カードは FF14 のパーティ募集と同様に "9マス + DH" のラインナップグリッドを表示。
  * 充足済スロット → チームロゴ入り＆緑枠／欠員スロット → "＋" アイコン＆赤枠。
* 募集作成フォームではチェックボックスで欠員ポジションを指定。保存するとグリッドに反映。
* ユーザは欠員スロットをクリック → 応募ダイアログがポップアップ。
* 応募すると `RecruitJoinRequest` が生成され、リアルタイムでカードの該当スロットが "申請中 (黄)" 表示に変化。
* チーム代表が承認するとスロットが緑色に変化しロック。

**リアルタイム性**
* ActionCable でカード状態をブロードキャストし、他ユーザの画面も即時更新。

**拡張性**
* ポジション定義テーブルで各スポーツ (野球以外) のロールも動的に追加可能 (サッカーの FW/MF/DF/GK など)。
